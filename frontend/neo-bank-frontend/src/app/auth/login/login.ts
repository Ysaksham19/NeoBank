...
export class Login implements OnInit, OnDestroy {
  ...
  loading        = signal(false);
  errorMessage   = signal('');
  sessionExpired = signal(false);  // ← amber banner
  logoutSuccess  = signal(false);  // ← green toast
  ...
  ngOnInit(): void {
    this.route.queryParams.subscribe(params => {
      ...
      // Session expired → amber banner (stays until dismissed or new login)
  ...
  onSubmit(): void {
    this.errorMessage.set('');
    this.sessionExpired.set(false);   // clear banners on new attempt
